	...
	$.ajax({
		...
		success: function (data) {
			...
			$.each(data, function (i) {
				...
				str += `<td class="actCategory">` + data[i].actCategory + `</td>`;
				str += `<td class="actName"><a href="/actDetail/` + data[i].actId + `">` + data[i].actName + `</a></td>`;
				str += `<td class="point">` + data[i].point + `</td>`;
				str += "</tr>";

				$("#actlist").empty();
				$("#statusCounts").empty();
				$('#stsListContainer').empty();
				$("#actlist").append(header);
				$("#actlist").append(str);
				$('#actContent').empty();
			})
		},
			...
			$.each(map.stsList, function (index, item) {
				...
				<div class="stateContainer">
	                ...
	                <div class="image-container">
	                	<img src="/resources/upload/${item.ctfcFilename}" height="90%"><br>
	                </div>
                </div>
                    <hr>
                `);
