...
public class ActivityController {
	...
	@GetMapping("/actList")	public String activityList(Model m) {
	...
	public String actDetail(@PathVariable String actNo, Model m) {
		List<String> ctgrList = aService.getCategoryAll();
		List<ActivityVO> actList = aService.getActivityAll();
	...
	public String actStart(Model m, @RequestParam("actno") int actno, HttpSession session) {
		String username = util.getCurrentUsername();
		String msg = "";
		...
		if (state == 1) {
			...
			msg = "동일한 활동은 하루에 한 번만 수행할 수 있습니다.";
			util.addMsgLoc(m, msg, loc);
		}	
		else {
	...
	@ResponseBody
	public String actCtfc(@RequestParam("actNo") int actno, 
			@RequestParam("imgFile") MultipartFile mfilename) {

        String upDir = ("/var/webapps/upload");
        UUID uuid = UUID.randomUUID();
        String fname = uuid.toString()+"_"+mfilename.getOriginalFilename();
