...
public class AdminServiceImpl implements AdminService {
	...
	public int approvalActivity(ActivityStateVO asVo) {
		...
		String startDate = asVo.getStartDate().substring(0,10);
		String userid = asVo.getUserid();
		int point;

		int isDailyMission = adMapper.checkMission(actId, startDate, userid);

		int n = adMapper.updateActivityState(stateId, 2);
	...
	@Override
	@Transactional
	public int retrievalPoint(ActivityStateVO asVo) {
		...
		int n = adMapper.updateActivityState(stateId, 4);
